...
export default function ExperienceCard(props: CommonItem) {
  ...
  const highlightedCourses = props.courses
    ? Object.entries(props.courses)
        .map(([key, courses]) => ({
          title: key,
          courses: courses.filter(
            (course) => typeof course !== "string" && course.highlight,
          ),
        }))
        ...
        <div className="flex flex-row justify-start flex-wrap gap-2 items-stretch">
          {props.tags?.map((tag, index) => (
            <Chip
              ...
              className={clsx(
                getColor(tag.type),
                getBorderColor(tag.type),
                "border flex flex-row gap-2 items-center justify-start text-xs",
              )}
            >
              {tag.title}
            </Chip>
        ...
        <div className="mt-2 w-full">
          ...
          {highlightedCourses.length > 0 && !showAll && (
            <div className="flex flex-wrap gap-1 text-sm text-white">
              {highlightedCourses.flatMap((group) =>
                group.courses.map((course, i) => {
                  if (typeof course === "string" || !course.highlight)
                    return null;
                  const inProgress =
                    typeof course !== "string" && course.inProgress;
                  const href =
                  ...
                  return (
                    ...
                    >
                      {inProgress && (
                        <Tooltip>
                          <TooltipTrigger>
                            <span className="w-1.5 h-1.5 bg-yellow-400 rounded-full inline-block ml-1" />
                          </TooltipTrigger>
                          <TooltipContent>
                            <p>In Progress</p>
                          </TooltipContent>
